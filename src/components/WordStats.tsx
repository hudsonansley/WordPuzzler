...
import {
  getWordleDisplayStats,
  wordleDisplayStatsType,
  wordleDisplayStatsKeys,
  WordSetInfoType,
  getIndexFromWord,
  WORDLE_CORRECT,
  WORDLE_WRONG_POSITION,
} from "../utilities/WordleUtils";
...
export const WordStats = ({ statsInfo }: { statsInfo: WordSetInfoType }) => {
  const [statsOrderInfo, setStatsOrderInfo] = useState<StatsOrderInfo>({
    primaryIndex: "avgGroupSize",
    targetWord: "",
    userWordChoices: [],
  });
  ...
  const combinedBoardIndexStringsRef = useRef(
  ...
  >(() => {
    ...
    if (
      ...
      combinedBoardIndexStringsRef.current !==
        statsInfo.combinedBoardIndexStrings
    ) {
    ...
    const newSortOrder = resetSortOrder
      ? sortOrder.current.slice()
      : initialSortOrder.slice();
    sortOrder.current = ArrayUtils.updatePrimaryIndex(
      newSortOrder,
      statsOrderInfo.primaryIndex
    ) as StatsSortOrder[];
    const result = getWordleDisplayStats(
      statsInfo,
      sortOrder.current,
      statsOrderInfo.targetWord,
      statsOrderInfo.userWordChoices
    );
    return result;
  ...
  const getRowClassName = (wordStats: wordleDisplayStatsType): string => {
    ...
    if (isNaN(boardNum)) {
      boardNum = statsInfo.wordSets.length;
    }
    return getBoardColorClass(
      boardNum,
      wordStats.cluesGroupDivider > 0,
      "group",
      "alt"
    );
  };
        ...
        <table className="stat-table">
            ...
            <tr>
              <th key="clues">
                <div className="clues-column">
                  <button
                    onClick={() => {
                      addWordToBoard(statsOrderInfo.targetWord);
                    }}
                  >
          ...
          <tbody>
            {wordleDisplayStats.map((wordInfo, i, stats) => {
              if (i > 0) {
                if (
                  wordInfo.clues === stats[i - 1].clues &&
                  wordInfo.boardGroup === stats[i - 1].boardGroup
                ) {
                  wordInfo.cluesGroupDivider = Math.sign(
                    stats[i - 1].cluesGroupDivider
                  );
                } else {
                  wordInfo.cluesGroupDivider =
                    -2 * Math.sign(stats[i - 1].cluesGroupDivider);
                }
              } else {
              ...
              return (
                <tr className={getRowClassName(wordInfo)} key={wordInfo.word}>
                  ...
                  <td key="avgGroupSize">
                    {getAvgGrpSizeDisplay(wordInfo.avgGroupSize)}
                  </td>
                  <td key="maxGroupSize">
