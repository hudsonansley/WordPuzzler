...
test("WordUtils.wordle properly reduces wordlist", () => {
  ...
  words = WordleUtils.wordle(
    picks,
    "l-a?t-e-r-_g-i?r-l-y-_d-r-o-l-l-_r-o-c-k-y-_q-u-a?s=i?_s?w-a?m?i?_s?p-i=c-e-"
  );
  expect(words.length).toEqual(1);
...
test("WordleUtils.getWordleDisplayStats works properly", () => {
  let wordInfo = {
    ...
    wordSetIndex: 0,
    combinedBoardIndexStrings: ["1", "2", "3,4", "4"],
    wordCount: 373,
  };
  const sortOrder = [
    {
...
test.skip("create WordleData file", () => {
  ...
  for (let i = 0; i < wordCount; i++) {
    data += `new Uint16Array([${WordleUtils.cluesLookUpTable[
      i
    ].toString()}]),\n`;
  }
  data += "];\n";
  ...
  for (let i = 0; i < wordCount; i++) {
    data += `new Uint16Array([${WordleUtils.groupSizesByClues[
      i
    ].toString()}]),\n`;
  }
  data += "];\n";
  ...
  require("fs").writeFile("./WordleData.js", data, function (err) {
    if (err) {
